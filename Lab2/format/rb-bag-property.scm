  ...
  (cond
    ...
    (else (let ((x1 (generate-random-list-with-f (random maxi) (random length)))
            ...
            (display-and-test
             ...
             (special-sort (lists-append x3 y3) <=)
             (string-append
...
(define (property-union-str tests-size maxi length iter)
  (cond
    ((<= tests-size 0) (display "All union tests finished\n"))
    (else (let ((x1 (generate-random-list-with-f-str
                     (random maxi)
                     (random length)))
                (y1 (generate-random-list-with-f-str
                     (random maxi)
                     (random length)))
                (x2 (generate-random-list-with-f-str
                     (random maxi)
                     (random length)))
                (y2 (generate-random-list-with-f-str
                     (random maxi)
                     (random length)))
                (x3 (generate-random-list-with-f-str
                     (random maxi)
                     (random length)))
                (y3 (generate-random-list-with-f-str
                     (random maxi)
                     (random length))))
            (display-and-test
             (null->f (rbmset->list (union-rbmset (rbmset-fill #f x1) y1)))
             (null->f (special-sort (null->f (lists-append x1 y1)) lower))
             (string-append
              "Property test for union (bag u list) number "
              (number->string iter)))
            (display-and-test
             (null->f (rbmset->list (union-rbmset x2 (rbmset-fill #f y2))))
             (null->f (special-sort (null->f (lists-append x2 y2)) lower))
             (string-append
              "Property test for union (list u bag) number "
              (number->string iter)))
            (display-and-test
             (null->f
              (rbmset->list
               (union-rbmset (rbmset-fill #f x3) (rbmset-fill #f y3))))
             (special-sort (null->f (lists-append x3 y3)) lower)
             (string-append
              "Property test for union (bag u bag) number "
              (number->string iter)))
  ...
  (cond
    ...
    (else (let ((x (generate-random-list-with-f-str
                ...
                (y (generate-random-list-with-f-str
                    ...
                    (random length))))
            (let ((left (null->f
                         (special-sort (null->f (lists-append x y)) lower)))
                  (right (null->f
                          (special-sort (null->f (lists-append y x)) lower)))
                  (midl (null->f
                         (rbmset->list
                          (union-rbmset (rbmset-fill #f x) (rbmset-fill #f y)))))
                  (midr (null->f
...
(define (property-delete-str tests-size maxi length iter)
  (cond
    ((<= tests-size 0) (display "All delete tests finished\n"))
    (else (let ((lst (generate-random-list-with-f-str
                      (random maxi)
                      (random length)))
                (del (generate-random-list-with-f-str
                      (random maxi)
                      (random length))))
            (display-and-assert
             (delete-and-compare (rbmset-fill #f lst) lst del)
