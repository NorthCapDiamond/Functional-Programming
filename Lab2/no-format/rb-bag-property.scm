	...
	(cond
		...
		[else
			(let ([x1 (generate-random-list-with-f (random maxi) (random length))]
				...
				(display-and-test (null->f (rbmset->list (union-rbmset (rbmset-fill #f x1) y1))) ( null->f (special-sort (lists-append x1 y1) <= )) (string-append "Property test for union (bag u list) number " (number->string iter)))
				(display-and-test (null->f (rbmset->list (union-rbmset x2 (rbmset-fill #f y2)))) ( null->f (special-sort (lists-append x2 y2) <= )) (string-append "Property test for union (list u bag) number " (number->string iter)))
				(display-and-test (null->f (rbmset->list (union-rbmset (rbmset-fill #f x3) (rbmset-fill #f y3)))) (null->f (special-sort (lists-append x3 y3) <= )) (string-append "Property test for union (bag u bag) number " (number->string iter)))
		...
		[else
			(let ([x (generate-random-list-with-f (random maxi) (random length))]
				...
				(let ([left (special-sort (lists-append x y) <= )]
					...
					(display-and-assert (and (equal? ( null->f left ) ( null->f midl)) (equal? ( null->f midl ) ( null->f midr )) (equal? ( null->f midr ) ( null->f right))) (string-append "Property test: list1-2 == list1 u list2 == list2 u list1 == list2-1 number " (number->string iter)))
					(property-union (- tests-size 1) maxi length (+ 1 iter))))]))
...
(define (property-monid tests-size maxi length iter op randf)
	(cond
		...
		[else
			(let ([a (generate-random-list-with-f (randf maxi) (randf length))]
				...
				(display-and-test (rbmset->list (op (op (rbmset-fill #f a) (rbmset-fill #f b)) (rbmset-fill #f c)))
													(rbmset->list (op (op (rbmset-fill #f b) (rbmset-fill #f c)) (rbmset-fill #f a)))
													(string-append "Property (a op b) op c == a op ( b op c ) number " (number->string iter)))
