...
(define (get-next app-list idx type)
	(cond
		[(eq? type 0)(let ([xy (read-x-y)])
			(list xy))]
		[else 
			(cond
				[(< idx (length (car app-list))) (list (list-ref (car app-list) idx))]
				[else (exit)])]))

(define (run-linear real-list update-list func step idx type)
	(cond
		[(< (length update-list) 2)(run-funcs-sub real-list update-list func step (+ 1 idx) type)]
		[else (display "Linear : ")(display (linear-function-step
			...
			(cadr (list-ref update-list (- (length update-list) 1))) step '()))
			(newline)]))

(define (run-lagrange real-list update-list func step idx type)
	(cond
		[(>= (/ (- (car (list-ref update-list (- (length update-list) 1))) (car (car update-list))) step) 5)
			(display "Lagrange : ")(display (lagrange-function-step update-list step '()))
			(newline)
			(run-funcs-sub real-list
			 (cdr update-list) func step (+ 1 idx) type)]
